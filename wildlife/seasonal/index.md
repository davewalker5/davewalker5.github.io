---
layout: default
title: Wildlife Through the Year In Abingdon
...
breadcrumb: Wildlife Through the Year
dataset: true
---

# Wildlife Through the Year In Abingdon

There are many ways to describe a year: by dates, weather, or familiar events. The Year in the Life analyses take a different approach — describing the year through repeated encounters with selected species, recorded over time and examined for seasonal pattern.

The pages collected here are built from long-term field observations gathered across many years. For each representative species included here, monthly records are used to explore how presence changes through the year: when a species becomes visible, when it reaches its seasonal peak, and when it fades again from regular observation.
...
The table below presents a curated selection of seasonal bird analyses, together with links to charts, fitted seasonal curves, classifications, and the underlying observational data used in the analyses.

{% include seasonal-table.html species=site.data.abingdon_yil.species type="Birds" %}
...
{% include seasonal-table.html species=site.data.abingdon_yil.species type="Butterflies" %}
...
{% include seasonal-table.html species=site.data.abingdon_yil.species type="Flora" %}
