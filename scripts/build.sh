...
source_plate_index="$MICROSCOPY_PLATE_LIBRARY/Index.xlsx"
...
if [[ $AIRCRAFT_BUILD_REQUIRED -eq 0 && $WILDLIFE_BUILD_REQUIRED -eq 0 && $WEATHER_BUILD_REQUIRED -eq 0  && $MICROSCOPY_BUILD_REQUIRED -eq 0 ]]; then
...
if [[ "$NO_UPDATE" != "true" && $MICROSCOPY_BUILD_REQUIRED -eq 1 ]]; then
    python "$PROJECT_ROOT/scripts/microscopy-library-builder.py" \
        -o "$target_plate_index" \
        -pod "$PROJECT_ROOT/assets/images/leitz"
fi
