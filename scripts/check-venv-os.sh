#!/usr/bin/env bash
# check_or_rebuild_venv.sh
# Usage: ./check_or_rebuild_venv.sh .venv python3

set -euo pipefail

export VENV_DIR=$( cd "$( dirname "$0" )/../venv" && pwd )
PY_CMD="${2:-python3}"
...
if [[ $needs_rebuild -eq 1 ]]; then
  echo "Virtual environment needs a rebuild because: $reason"
fi
