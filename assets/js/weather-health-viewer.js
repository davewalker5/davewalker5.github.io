(function () {
  ...
  if (parts.length < 4) {
    heading.textContent = "Sensor Health Report";
  ...
  const pngSrc = `${baseurl}/weather/reports/health/${base}.png`;
  const xlsxSrc = `${baseurl}/weather/reports/health/${base}.xlsx`;
  ...
  document.title = `Sensor Health Report: ${sensorLabel} — ${measurementLabel} — ${metricLabel}`;
  heading.textContent = `${sensorLabel} — ${measurementLabel}`;
  intro.textContent = metricLabel;
  meta.textContent = "Sensor health report generated from personal weather station records.";

  img.src = pngSrc;
  img.alt = `Sensor health report ${sensorLabel} ${measurementLabel} ${metricLabel}`;
