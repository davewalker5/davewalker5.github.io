(function () {
  ...
  if (parts.length < 4) {
    heading.textContent = "Daily Weather Report";
  ...
  const pngSrc = `${baseurl}/weather/reports/daily/${base}.png`;
  const xlsxSrc = `${baseurl}/weather/reports/daily/${base}.xlsx`;
  ...
  document.title = `Daily Weather Report: ${sensorLabel} — ${measurementLabel} — ${metricLabel}`;
  heading.textContent = `${sensorLabel} — ${measurementLabel}`;
  intro.textContent = metricLabel;
  meta.textContent = "Daily weather report generated from personal weather station records.";

  img.src = pngSrc;
  img.alt = `Daily weather report ${sensorLabel} ${measurementLabel} ${metricLabel}`;
